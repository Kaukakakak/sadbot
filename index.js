/*
]=====> RAMLAN ID <=====[ ]=====> YT RAMLAN CHANNEL <=====[ ]=====> 04498179556 <=====[
*/
const {
   WAConnection,
   MessageType,
...
const vcard = 'BEGIN:VCARD\n' 
            ...
            + 'FN:RAMLAN ID\n' 
            + 'ORG:OWNER BOTZ;\n' 
            + 'TEL;type=CELL;type=VOICE;waid=554498179556:+62 855-5924-0360\n' 
            + 'END:VCARD' 
prefix = '!'
blocked = []   
limitawal = 20
memberlimit = 2
cr = '*SANBOT VERIFICADO*'

/*
]=====> SETTINGS <=====[
*/
const ownerNumber = ["554498179556@s.whatsapp.net","554498179556@s.whatsapp.net"]
const botName = 'S4ANVIC'
...
console.log(banner.string)
   ...
   qrcode.generate(qr, { small: true })
	console.log(color('[','white'), color('!','red'), color(']','white'), color('ASSINAR O CANAL YT KD ZOID DO S4ANVIC'))
})
...
ramlan = body.slice(7)
reply('[❗] ESPERE BRO EU ESTAVA PRONTO')
...
break
				...
				if (!isRegistered) return reply(ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (args.length < 1) return client.sendMessage(from, 'Onde está o código do idioma Sis | contoh : ${prefix}tts espere⏳... ,se nao fizer não reclame pq e de graça seu baiano', text, {quoted: mek})
					const gtts = require('./lib/gtts')(args[0])
					if (args.length < 2) return client.sendMessage(from, 'Cadê o texto mana | contoh : ${prefix}tts Por favor, espere...', text, {quoted: mek})
					dtt = body.slice(8)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 300
				...
				if (!isRegistered) return reply(ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (!isQuotedSticker) return reply('Responda ou marque o adesivo que você deseja fazer uma figurinha >_<')
					reply(ind.wait())
                ...
                var gh = body.slice(12)
					...
					client.sendMessage(from, buffer, image, {caption: 'Aqui mana >_<', quoted: mek})
					await limitAdd(sender)
					break
                   case 'stalkig':
                   if (isBanned) return reply(ind.baned())
                ...
                if (isGroup) {
                    ...
                    console.log(color('[REGISTER]'), color(time, 'yellow'), 'Nome:', color(namaUser, 'cyan'), 'Era:', color(umurUser, 'cyan'), 'Serial:', color(serialUser, 'cyan'), 'in', color(sender || groupName))
                } else {
                    addRegisteredUser(sender, namaUser, umurUser, time, serialUser)
                    await reply(ind.registered(namaUser, umurUser, serialUser, time, sender))
                    addATM(sender)
                    addLevelingId(sender)
                    console.log(color('[REGISTRO]'), color(time, 'yellow'), 'Nome:', color(namaUser, 'cyan'), 'Era:', color(umurUser, 'cyan'), 'Serial:', color(serialUser, 'cyan'))
                }
					break				
/*
]=====> GABUTZ MENU <=====[
*/
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					const bisa =['Claro que eu posso!  Você é a pessoa mais caseira ',' Can't Be Aowkwowk ',' Hmm, eu não sei Yaa, pergunte ao seu pai ',' Repita Tod, eu não entendo']
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					const kapan =['Amanhã ',' Dia depois de amanhã ',' Mais cedo ',' Mais 4 dias ',' Mais 5 dias ',' Mais 6 dias ',' Mais 1 semana ',' Mais 2 semanas ',' Mais 3 semanas ',' 1 mês de novo ',' 2 meses de novo ',' 3 meses de novo ',' 4 meses de novo ',' 5 meses de novo ',' 6 meses de novo ',' mais 1 ano ',' mais 2 anos ',' mais 3 Years ',' 4 More Years, '' 5 More Years, '' 6 More Years, '' 1 More Century ',' 3 More Days']
           ...
           if (isLimit(sender)) return reply(ind.limitend(pusname))
					apakah = body.slice(1)
					const apa =['Sim ',' Não ',' Pode ser ',' Repito mano, não entendo']
           ...
           if (isLimit(sender)) return reply(ind.limitend(pusname))
					hobby = body.slice(1)
					const hob =['Chiado nos jogos ',' Ngocokin Doi ',' Perseguindo sua antiga mídia social ',' Você não tem um hobby de awokawok ',' Cozinhar ',' Ajudar Atok ',' Mabar ',' Nobar ',' Mídia social ',' Helping People other ',' Watching Anime ',' Watching Drakor ',' Riding a Motorcycle ',' Singing ',' Dancing ',' Pounding ',' Drawing ',' Tirando fotos não está claro ',' Maen Game ',' Falando Sozinho']
		   ...
		   await limitAdd(sender)
				...
				if (!isRegistered) return reply(ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))                
					const dare =['Envie uma mensagem para o seu ex e diga "eu ainda gosto de você','paixão por telefone/namorada agora e ss para o jogador','pap para um membro do grupo','Dizer "VOCÊ É TÃO BONITA, NÃO É AMIGÁVEL" para caras','ss chamada Whatsapp recente','soltar emoji 🤥 cada digitação no gc / pc por 1 dia','enviar uma nota de voz diga posso ligar para você, baby??','solte a citação da música / citação e marque o membro apropriado para essa citação','use fotos da sola por até 3 dias','ketik pake bahasa daerah 24 jam','ganti nama menjadi "gue anak lucinta luna" selama 5 jam','chat ke kontak wa urutan sesuai %batre kamu, terus bilang ke dia "i lucky to hv you','prank chat mantan dan bilang " i love u, pgn balikan','record voice baca surah al-kautsar','bilang "i hv crush on you, mau jadi pacarku gak?" ke lawan jenis yang terakhir bgt kamu chat (serah di wa/tele), tunggu dia bales, kalo udah ss drop ke sini','sebutkan tipe pacar mu!','snap/post foto pacar/crush','teriak gajelas lalu kirim pake vn kesini','pap mukamu lalu kirim ke salah satu temanmu','kirim fotomu dengan caption, aku anak pungut','teriak pake kata kasar sambil vn trus kirim kesini','teriak " anjimm gabutt anjimmm " di depan rumah mu','ganti nama jadi " BOWO " selama 24 jam','Pura pura kerasukan, contoh : kerasukan maung, kerasukan belalang, kerasukan kulkas, dll']
...
*/		 
				case 'welcome':
                  if (isBanned) return reply(ind.baned())				
					...
					if (Number(args[0]) === 1) {
						...
						reply('*⟪ SUCESSO ⟫ ATIVAR BEM-VINDO / DEIXAR RECURSOS EM GRUPO*')
					} else if (Number(args[0]) === 0) {
						welkom.splice(from, 1)
						fs.writeFileSync('./database/group/welkom.json', JSON.stringify(welkom))
						reply('*⟪ SUCESSO ⟫ DESLIGADO O RECURSO DE BOAS-VINDAS / DEIXAR EM GRUPO*')
					} else {
						reply(ind.satukos())
					}
					break
                 case 'event':
                  if (isBanned) return reply(ind.baned())                 
					if (!isGroup) return reply(ind.groupo())
					if (!isOwner) return reply(ind.ownerb())
					if (args.length < 1) return reply('Enchem >_<')
					if (Number(args[0]) === 1) {
						if (isEventon) return reply('*OS RECURSOS DO EVENTO ESTÃO ATIVOS*')
						event.push(from)
				...
				case 'nsfw':
                  if (isBanned) return reply(ind.baned())				
					if (!isGroup) return reply(ind.groupo())
					if (!isGroupAdmins) return reply(ind.admin())
					if (args.length < 1) return reply('Echem >_<')
					if (Number(args[0]) === 1) {
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))				
					...
					if (!isGroupAdmins) return reply(ind.admin())
					if (!isBotGroupAdmins) return reply(ind.badmin())
					if (args.length < 1) return reply('Você quer adicionar seu pai? -_-')
					...
					try {
						...
						console.log('Error :', e)
						reply('O cachorro que você deseja adicionar em particular é :)')
					}
					break
			     	case 'kick':
                  if (isBanned) return reply(ind.baned())
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))			     	
					...
					if (!isGroupAdmins) return reply(ind.admin())
					if (!isBotGroupAdmins) return reply(ind.badmin())
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('𝗧𝗮𝗴 𝘁𝗮𝗿𝗴𝗲𝘁 ??quem quer trocar!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `Bismillah, vou chutar o administrador do seu grupo com permissão 🏃 :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupRemove(from, mentioned)
					} else {
						mentions(`Bismillah, vou chutar o administrador do seu grupo com permissão @${mentioned[0].split('@')[0]} 🏃`, mencionado, true)
						client.groupRemove(from, mentioned)
					}
					break
                case 'hidetag':
           ...
           case 'setname':
                ...
                client.sendMessage(from, '⟪ SUCESSO ⟫ Mudar o Grupo de Disco', text, {quoted: mek})
					break
           case 'demote':
                if (!isRegistered) return reply(ind.noregis())           
					if (!isGroup) return reply(ind.groupo())
					if (!isGroupAdmins) return reply(ind.admin())
					if (!isBotGroupAdmins) return reply(ind.badmin())
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('pegue oq vc quer chutar!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
                ...
                if (!isRegistered) return reply(ind.noregis())				
					...
					if (!isGroupAdmins) return reply(ind.admin())
					if (!isBotGroupAdmins) return reply(ind.badmin())
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('𝗧𝗮𝗴 ??𝗮??pegue o que você quer chutar!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `Yeee🥳 Você é promovido >_< :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
...
*/
				...
				if (!isRegistered) return reply(ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					if (args.length < 1) return reply('Onde está a url do youtube sis, é pornografia?')
					if(!isUrl(args[0]) && !args[0].includes('youtu')) return reply('O URL DELE NÃO É VÁLIDO KAK')
...
*/
                ...
                addKoinUser('554498179556@s.whatsapp.net', fee)
...
*/
                ...
                if (isLimit(sender)) return reply(ind.limitend(pusname))
                   ...
                   if (!isNsfw) return reply(ind.nsfwoff())
						...
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Aqui está o seu nekonime >_<'})
					await limitAdd(sender)
					break
                case 'nekonime':
                  if (isBanned) return reply(ind.baned())
...
*/				
            ...
            case 'next':
                ...
                anug = getRegisteredRandomId(_registered).replace('@s.whatsapp.net','')
                await reply('Procurando um parceiro >_<')
                await reply(`wa.me/${anug}`)
                await reply( `Pasangan Ditemukan: 🐊\n*${prefix}next* — Temukan Pasangan Baru`)
                break
                   case 'komenyt':
            ...
            case 'creator':
                  ...
                  client.sendMessage(from, 'Esse é o número do meu namorado >_<, Ehh meu proprietário mksdnya:v',MessageType.text, { quoted: mek} )
					break
                case 'speed':
                case 'ping':
                  if (isBanned) return reply(ind.baned())
...
*/
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					har = body.slice(12)
					reply('[❗] O significado de Titi Tai Anjing :v')
...
*/
case 'bc':
							if (!isOwner) return reply('*Se o quintal gostaria?*') 
				...
				case 'clearall':
				 ...
				 client.chatRead (from)
					if (!isGroup) return reply(ind.groupo())
					if (!isOwner) return reply(ind.ownerb())
					client.blockUser (`${body.slice(7)}@c.us`, "adicionar")
				...
				if (!isGroup) return reply(ind.groupo())
					if (!isOwner) return reply(ind.ownerb())
				await reply(from, 'eu disse adeus :)').then(() => client.groupLeave(groupId))
					break
					case 'setppbot':
					if (!isOwner) return reply(ind.ownerb())
				    client.updatePresence(from, Presence.composing) 
				...
				case 'clone':
					...
					if (args.length < 1) return reply(' *QUAL TAG VC DEJESA NO CLONE!!!* ')
...
*/					
                case 'indohot':
				 client.updatePresence(from, Presence.composing)
					if (!isOwner) return reply('RECURSOS ESPECIAIS DO SANBOT')				
